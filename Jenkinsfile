...
pipeline {
    ...
    stages {
        ...
        stage ('test sonar analysis') {
            steps {
                sh 'mvn clean verify sonar:sonar -Dsonar.projectkey=second-org-b2-b3-key -Dsonar.organization=second-org-b2-b3-key -Dsonar.host.url=https://sonarcloud.io -Dsonar.login=c8909b8bb08e38a054ba3b4ccb2673f46c2087e5'
                echo "report save successfully"
            }
        }
        stage ('deploy stage') {
            steps {
                deploy adapters: [tomcat9(credentialsId: 'tomcat-cred', path: '', url: 'http://51.20.41.203:8080')], contextPath: '/', war: '**/*.war'
                echo "app depoly successfully"
            }
        }
    }   
